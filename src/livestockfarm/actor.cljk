(ns livestockfarm.actor
  "LivestockFarmActor — the ISCO-08 9212 livestock-farm scheduling/
  logistics coordination actor as a `langgraph.graph/state-graph`
  (ADR-2607121000 / CLAUDE.md Actors section). One graph run = one farm-
  ...
  run can resume after human sign-off. Modeled closely on
  cloud-itonami-isco-7511's butcher.actor for the animal-material-
  handling hazard-domain shape, extended with a second, independent
  outdoor-exposure/farm-equipment hazard-scope dimension.
  ...
  The unconditional invariant: the Livestock Farm Labourer Advisor can
  never directly commit an operating record the LivestockFarmGovernor
  refuses, and can never make this actor finalize a livestock-handling-
  execution decision (e.g. approving a specific handling or restraint
  procedure) or a farm-safety-clearance decision (e.g. declaring a farm
  cleared for safety), and can never override a farm safety supervisor's
  ...
  regardless of disposition path. This actor coordinates FARM
  SCHEDULING/LOGISTICS ONLY — it never handles livestock or performs
  farm labour work itself and never makes a farm-safety-clearance
  decision itself."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [livestockfarm.advisor :as advisor]
            [livestockfarm.governor :as governor]
            [livestockfarm.store :as store]))

(defn build-graph
  "Build a compiled LivestockFarmActor graph. `store` implements
  `livestockfarm.store/Store`. `advisor` implements
  `livestockfarm.advisor/Advisor` (defaults to `mock-advisor`).
